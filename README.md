...
| `--log-retention N` | `ABLOCKER_LOG_RETENTION` | сколько архивов лога Xray хранить |
...
Инсталлер настраивает ротацию `access.log` Xray: ротация при 100 МБ, архивы
жмутся в gzip, старые удаляются. Сколько архивов хранить — спрашивается при
установке (или задаётся флагом `--log-retention N`):

| Архивов | Примерно диска | Когда |
|---|---|---|
| 3 | ~0.2 ГБ | VPS с маленьким диском; логи живут часы |
| 24 (по умолчанию) | ~1.5 ГБ | примерно сутки |
| 168 | ~10 ГБ | примерно неделя — для дедиков с большим диском |

Оценки для нагруженной ноды; сколько это по времени, зависит от её трафика.
Длинное хранение нужно, чтобы по abuse-жалобе (DMCA и т.п.) успеть найти
нарушителя: жалобы часто приходят через сутки после события, а на нагруженной
ноде трёх архивов хватает лишь на несколько часов.

Сменить на уже работающей ноде — повторным запуском инсталлера:

```bash
bash ablocker-install.sh -y --log-retention 168
```

Конфиг ротации: `/etc/logrotate.d/remnanode`, запускается systemd-таймером
каждый час (cron не нужен).

## Ложные срабатывания malware-фидов

IP-фиды содержат адреса, за которыми малварь прячется через Cloudflare и другие
общие CDN. Такой адрес обслуживает тысячи легитимных сайтов, поэтому бан клиента
за обращение к нему — гарантированный ложняк. ablocker выкидывает диапазоны
Cloudflare и публичных DNS-резолверов из IP-фидов при загрузке (в логе:
`skipped N shared-CDN/anycast ip(s)`); домены таких угроз при этом остаются в
блок-листе. Свои диапазоны можно добавить через `MalwareIPExclude`:

```yaml
MalwareIPExclude:
  - "203.0.113.0/24"    # напр. собственные выходные ноды
```

Важно: `MalwareIPExclude` — про адрес **назначения**, а `BypassIPS` (ниже) — про
IP **клиента**. Это разные списки.

## Белый список (BypassIPS)

IP и подсети из `BypassIPS` **никогда не банятся** — ни за торрент, ни за malware.
Проверка идёт до бана и переживает рестарт. Поддерживаются одиночные IP и
CIDR-подсети (IPv4 и IPv6). В `/opt/ablocker/config.yaml`:

```yaml
BypassIPS:
  - "203.0.113.10"     # одиночный IP (напр. свой личный или мониторинг)
  - "10.8.0.0/16"      # целая подсеть
  - "2001:db8::/32"    # IPv6-подсеть
```

После правки — `systemctl restart ablocker`. Проверить, что список подхватился:
`journalctl -u ablocker --no-pager | grep -A20 "Bypass IPS list"`.

Разовый разбан уже забаненного IP — это другое, для него `ablocker-unban <IP>`.
